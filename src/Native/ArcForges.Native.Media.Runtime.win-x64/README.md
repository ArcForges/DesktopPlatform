# ArcForges.Native.Media.Runtime.win-x64

Actual Windows x64 `ArcMediaNative.dll`, its non-system DLL dependencies, public C headers and import
library. Pair with `ArcForges.Native.Media` at the same exact version and set `RuntimeIdentifier` to `win-x64`.
