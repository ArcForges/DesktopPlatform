# ArcForges.Native.Otio.Runtime.win-x64

Actual Windows x64 `ArcSlateOtioNative.dll`, its non-system DLL dependencies, public C headers and import
library. Pair with `ArcForges.Native.Otio` at the same exact version and set `RuntimeIdentifier` to `win-x64`.
